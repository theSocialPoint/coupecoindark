...
// Copyright (c) 2013-2017 The Coupecoindark developers
...
#if defined(NDEBUG)
# error "Coupecoindark cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Coupecoindark Signed Message:\n";
...
{
    // Coupecoindark: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 12 * 60 * 60; // Coupecoindark: 12 hours
static const int64 nTargetSpacing = 60; // Coupecoindark: 60 seconds
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespan2Q13 = 60 * 60; // Coupecoindark: 60 minutes
...
unsigned int static GetNextWorkRequiredCoupecoindark(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Coupecoindark: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    ...
    int64_t PastBlocksMin = 60; // Coupecoindark: look-back 60 minutes
    int64_t PastBlocksMax = 60; // Coupecoindark: look-back 60 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    } else if (retarget == DIFF_CDARK) {
        // Retarget using Coupecoindark style retargeting
        return GetNextWorkRequiredCoupecoindark(pindexLast, pblock);
    } else  {
        // Retarget using Classic Coupecoindark style retargeting
        unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
...
void ThreadScriptCheck() {
    RenameThread("coupecoindark-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Coupecoindark: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x650b1f6b951676907cf162ec685c8a76723ef97107ce3373b48f235d46c6411e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04015366184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
...
unsigned char pchMessageStart[4] = { 0x63, 0x42, 0x21, 0x2c }; // Coupecoindark: switched at 1393657079.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CoupecoindarkMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CoupecoindarkMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CoupecoindarkMiner(CWallet *pwallet)
{
    printf("CoupecoindarkMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coupecoindark-miner");
    ...
    try { loop {
        ...
        printf("Running CoupecoindarkMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CoupecoindarkMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CoupecoindarkMiner, pwallet));
}
